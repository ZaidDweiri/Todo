 ...
 * Ensures the task belongs to the current user for security.
 */
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    // Step 3: Get the current user ID from Clerk authentication
    ...
    // Step 5: Check if the task exists
    if (!task) {
      return NextResponse.json({ error: "Task not found" }, { status: 404 })
    }

    // Step 6: Check if the task belongs to the current user
    if (task.userId !== await userId) {
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (existingTask.userId !== await userId) {
